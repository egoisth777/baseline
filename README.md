...
2. Establishes the **config folder** — your routes + docs (`config.json` + `docs/`). It lives wherever you point `BASELINE_CFG` (else `~/.baseline/cfg`); when `BASELINE_CFG` is set, `~/.baseline/cfg` is a symlink to it. If empty, it is seeded from a repo **preset** — `minimal` by default (`--preset default` for the curated baseline).
3. **Links** each agent's config dir back into the install root — for example `~/.claude/hooks/baseline-recital.js` and `~/.codex/hooks/baseline-recital.js` → the dispatcher, with each agent's `cfg/baseline/` → `~/.baseline/cfg` (your config folder).
4. Deploys a **Claude skills-dir plugin** to `~/.baseline/skills/baseline/` (a `.claude-plugin/plugin.json` + a `SKILL.md` wrapper) and links it into Claude's skills dir (`~/.claude/skills/baseline/`), so Claude recognizes baseline as `baseline@skills-dir` and loads its control-surface skill. It carries **no hooks** — it only makes baseline discoverable, not active. Codex uses its own `.codex-plugin/` manifest instead.
...
- `presets/default/` — the curated baseline (multi-route). Opt-in: `install --preset default`.
