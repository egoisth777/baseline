...
function setCentralConfig(omne, routes, docs) {
...
function testCentralInstallAndAgentLinks() {
    ...
    assert.strictEqual(readThrough(agentJs), readThrough(path.join(omne, 'hooks', 'baseline-recital.js')), 'agent dispatcher does not resolve to central content');
    assert.strictEqual(readThrough(agentConfig), readThrough(path.join(omne, 'cfg', 'baseline', 'config.json')), 'agent config.json does not resolve to central content');
...
function testForceReplacesConfigKeepWithout() {
    ...
    assert.strictEqual(run(['install'], cfg).status, 0);
    assert.strictEqual(fs.readFileSync(path.join(omne, 'cfg', 'baseline', 'docs', 'baseline.md'), 'utf8'), marker, 'plain reinstall clobbered the operator config');
    ...
    assert.notStrictEqual(fs.readFileSync(path.join(omne, 'cfg', 'baseline', 'docs', 'baseline.md'), 'utf8'), marker, '--force did not replace the operator config');
}
