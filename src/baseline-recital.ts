...
function loadValidRoutes(): Route[] {
  ...
  for (const r of routes) {
    ...
    if (typeof r.event !== 'string' || SUPPORTED_EVENTS.indexOf(r.event) === -1) continue;
    ...
    if (r.matcher !== undefined && typeof r.matcher !== 'string') continue;
...
function matcherMatches(route: Route, data: HookInput): boolean {
  if (route.matcher === undefined) return true;
  if (route.event === 'UserPromptSubmit') return true;
  if (route.event === 'SessionStart') return data.source === route.matcher;
  const tool = typeof data.tool_name === 'string' ? data.tool_name : '';
  try { return new RegExp(route.matcher).test(tool); } catch (e) { return false; }
}
...
process.stdin.on('end', () => {
  try {
    ...
    const selected = loadValidRoutes().filter(r =>
      r.event === event && matcherMatches(r, data) && cwdMatches(r, data));
